...
#            f_sh.write('%s\n' % ('cd $PBS_O_WORKDIR'))
            ...
            out_file.append(dir_gen+'/BSUB_'+runID+'_'+self.dir_coadds[0]+'.sh')
...
#            f_sh.close()
            ...
            f_qsub = open(dir_gen+'/SH_'+runID+'_'+self.dir_coadds[0]+'.sh', 'w')
            f_qsub.write('%s\n' % ('#!/bin/sh'))
#            f_qsub.write('%s\n' % ('python '+dir_simulator+'/src/run_coaddmaps.py '+out_dir+' '+nside+' "'+sqlite_command+'"& '))
            f_qsub.write('%s\n' % ('python '+dir_simulator+'/src/run_coaddmaps.py '+self.outdirs[0]+' '+self.dir_coadds[0]+' '+nside+' "'+sqlite_command+'" '+xml_filename+' & '))
            f_qsub.write('%s\n' % ('wait'))
            f_qsub.close()

            f_TQU = open(dir_gen+'/SH_'+runID+'_'+self.dir_coadds[0]+'_TQU.sh', 'w')
            f_TQU.write('%s\n' % ('#!/bin/sh'))
            f_TQU.write('%s\n' % ('python '+dir_simulator+'/src/fits_sep2oneTQU.py ' \
                                      +self.outdirs[0]+'/coadd_map/'+self.dir_coadds[0]+'/mapT.fits ' \
                                      +self.outdirs[0]+'/coadd_map/'+self.dir_coadds[0]+'/mapQ.fits ' \
                                      +self.outdirs[0]+'/coadd_map/'+self.dir_coadds[0]+'/mapU.fits ' \
                                      +self.outdirs[0]+'/coadd_map/'+self.dir_coadds[0]+'/mapTQU.fits')) 
            f_TQU.write('%s\n' % ('rm -f '+self.outdirs[0]+'/coadd_map/'+self.dir_coadds[0]+'/*.npy'))  
            f_TQU.write('%s\n' % ('wait'))
            f_TQU.close()
            
            os.system('chmod 744 '+dir_gen+'/SH_'+runID+'_'+self.dir_coadds[0]+'.sh')
            os.system('chmod 744 '+dir_gen+'/SH_'+runID+'_'+self.dir_coadds[0]+'_TQU.sh')
